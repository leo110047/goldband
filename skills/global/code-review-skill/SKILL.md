...
1. STOP the code review immediately
2. Invoke the `systematic-debugging` skill
3. Let systematic debugging complete its investigation
4. Only resume review after bugs are fixed

**When you find performance issues during review:**
- Flag them in review comments
- Recommend using the `performance-optimization` skill
...
|-------------------|----------------|------------|
| **C#** | [C# Guide](reference/csharp.md) | .NET 8+, LINQ, async/await, Span/Memory, EF Core, DI, Nullable Reference Types |
| **Python** | [Python Guide](reference/python.md) | 类型注解, async/await, 可变默认参数, 异常处理 |
| **TypeScript** | [TypeScript Guide](reference/typescript.md) | 类型安全, 泛型, Strict 模式, async/await |
