...
function evaluateContextWarning(input) {
  ...
  const severity = count >= criticalThreshold
    ? 'CRITICAL'
    : count >= warnThreshold
      ? 'WARNING'
      : 'none';
  ...
  const debounced = severity !== 'none' && (count - state.lastNotifyCount) >= CONTEXT_NOTIFY_INTERVAL;
...
function evaluatePostToolUse(input) {
  ...
  const consoleLogs = toolName === 'Edit'
    ? collectConsoleWarnings(filePath)
    : [];
  ...
  return {
    ...
    blockedBy: null,
    logs: [...consoleLogs],
    outputJson: buildAdditionalContext(contextMessage),
    spawnedProcesses: 0
  };
}
