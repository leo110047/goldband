#!/usr/bin/env bun
/**
 * analytics — CLI for viewing workflow skill usage statistics.
 *
 * Reads ~/.workflow/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.workflow', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('workflow skill usage analytics');
