...
echo '{"skill":"qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.workflow/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
```bash
~/.claude/skills/workflow/bin/workflow-question-log '{"skill":"qa","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
| Mode | full | `--regression .workflow/qa-reports/baseline.json` |
| Output dir | `.workflow/qa-reports/` | `Output to /tmp/qa` |
...
```bash
mkdir -p .workflow/qa-reports/screenshots
```
...
1. **Project-scoped test plans:** Check `~/.workflow/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   eval "$(~/.claude/skills/workflow/bin/workflow-slug 2>/dev/null)"
   ls -t ~/.workflow/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
- Make the **healthiest complete fix** that resolves the issue and avoids leaving obvious recurrence behind
...
- Include full attribution comment:
  ...
  // Report: .workflow/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.workflow/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:** Write test outcome artifact for cross-session context:
```bash
eval "$(~/.claude/skills/workflow/bin/workflow-slug 2>/dev/null)" && mkdir -p ~/.workflow/projects/$SLUG
```
Write to `~/.workflow/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
