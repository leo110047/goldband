...
echo '{"skill":"benchmark","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.workflow/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
mkdir -p .workflow/benchmark-reports
mkdir -p .workflow/benchmark-reports/baselines
```
...
Write to `.workflow/benchmark-reports/baselines/baseline.json`.
...
Write to `.workflow/benchmark-reports/{date}-benchmark.md` and `.workflow/benchmark-reports/{date}-benchmark.json`.
