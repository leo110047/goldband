...
3. **Question logging** (`bin/workflow-question-log`). Appends `{ts, question_id, user_choice, recommended, session_id}` to `~/.workflow/projects/{SLUG}/question-log.jsonl`. Validates against registry.
4. **Explicit per-question preferences** (`bin/workflow-question-preference`). Writes `{question_id, preference}` where preference is `always-ask | never-ask | ask-only-for-one-way`. Respected from session 1. No calibration gate — user stated it, system obeys.
5. **Preamble injection.** Before each AskUserQuestion, agent calls `workflow-question-preference --check <registry-id>`. If `never-ask` AND question is NOT a one-way door, auto-choose recommended option with visible annotation: "Auto-decided [summary] → [option] (your preference). Change with /plan-tune." One-way doors always ask regardless of preference — safety override.
...
7. **Declared profile** (`/plan-tune setup`). 5 plain-English questions, one per dimension. Stored in unified `~/.workflow/developer-profile.json` under `declared: {...}`. Informational only in v1 — no skill behavior change.
...
10. **Unification with existing `~/.workflow/builder-profile.jsonl`.** Fold /office-hours session records and accumulated signals into unified `~/.workflow/developer-profile.json`. Migration is atomic + idempotent + archives the source file.
...
2. Before each AskUserQuestion:
   - Agent calls `workflow-question-preference --check <registry-id>`
...
5. If user's NEXT turn message contains `tune:` prefix AND the content originated in the user's own message (not tool output):
   - Agent calls `workflow-question-preference --write` with `source: "inline-user"`
   - Binary validates source field; rejects if anything other than `inline-user`
6. Inferred dimensions recomputed on demand by `bin/workflow-developer-profile --derive`. Signal map changes trigger full recompute from events history.
...
Binary enforcement: `workflow-question-preference --write` requires `source: "inline-user"` field on every tune-originated record. Any other source value (e.g., `inline-tool-output`, `inline-file-content`) is rejected with an error. Agent is instructed to never forge the `source` field.

**Data privacy**:
- All data is local-only under `~/.workflow/`. Nothing leaves without explicit user action.
- `/plan-tune export <path>` writes profile to user-specified path (opt-in export).
- `/plan-tune delete` wipes local profile files.
- `workflow-config set telemetry off` prevents any telemetry (this skill never sends profile data regardless).
- Profile files have standard user-home permissions.

**Injection defense** (consistent with existing `bin/workflow-learnings-log` patterns): the `question_summary` and any free-form user feedback fields are sanitized against known prompt-injection patterns ("ignore previous instructions," "system:", etc.).
...
5. **Per-project preferences beat global preferences.** `~/.workflow/projects/{SLUG}/question-preferences.json` wins over any future global preference file. Global profile (`~/.workflow/developer-profile.json`) is a starting point for diversity across projects.
...
### Binaries
- `bin/workflow-question-log` — append log record, validate against registry.
- `bin/workflow-question-preference` — read/write/check/clear explicit preferences.
- `bin/workflow-developer-profile` — supersedes `bin/gstack-builder-profile`. Subcommands: `--read` (legacy compat), `--derive`, `--gap`, `--profile`.
...
- `bin/gstack-builder-profile` — legacy shim delegates to `bin/workflow-developer-profile --read`.
...
- Migration from `~/.workflow/builder-profile.jsonl` preserves 100% of sessions + signals_accumulated. Regression test with 7-session fixture.
...
6. `bin/workflow-question-log` — validates against registry, rejects unknown IDs.
7. `bin/workflow-question-preference` — all subcommands + tests.
8. `bin/workflow-developer-profile` — `--read` (legacy), `--derive`, `--gap`, `--profile`.
