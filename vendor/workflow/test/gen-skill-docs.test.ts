...
const ALL_SKILLS = (() => {
  ...
  if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
    skills.push({ dir: '.', name: 'root workflow' });
  }
...
describe('gen-skill-docs', () => {
  ...
  test('generated SKILL.md contains contributor mode check', () => {
    ...
    expect(content).toContain('workflow_contributor');
  ...
  test('generated SKILL.md contains telemetry line', () => {
    ...
    expect(content).toContain('~/.workflow/analytics');
  });
  ...
  test('preamble .pending-* glob is zsh-safe (uses find, not shell glob)', () => {
    for (const skill of ALL_SKILLS) {
      ...
      expect(content).toContain("find ~/.workflow/analytics -maxdepth 1 -name '.pending-*'");
    }
  });
...
describe('PLAN_FILE_REVIEW_REPORT resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`plan file review report appears in ${skill} generated file`, () => {
      const content = fs.readFileSync(path.join(ROOT, skill, 'SKILL.md'), 'utf-8');
      expect(content).toContain('WORKFLOW REVIEW REPORT');
    });
  }
...
describe('Plan status footer in preamble', () => {
  test('preamble contains plan status footer', () => {
    ...
    expect(content).toContain('WORKFLOW REVIEW REPORT');
    expect(content).toContain('workflow-review-read');
...
describe('CODEX_SECOND_OPINION resolver', () => {
  const content = fs.readFileSync(path.join(ROOT, 'office-hours', 'SKILL.md'), 'utf-8');
  const codexContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'workflow-office-hours', 'SKILL.md'), 'utf-8');
  ...
  test('Codex host variant does NOT contain the Phase 3.5 resolver output', () => {
    ...
    expect(codexContent).not.toContain('workflow-codex-oh-');
  });
});
...
describe('Codex generation (--host codex)', () => {
  ...
  const CODEX_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      skills.push({ dir: '.', codexName: 'workflow' });
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      if (entry.name === 'codex') continue; // /codex is excluded from Codex output
      if (!fs.existsSync(path.join(ROOT, entry.name, 'SKILL.md.tmpl'))) continue;
      const codexName = entry.name.startsWith('workflow-') ? entry.name : `workflow-${entry.name}`;
  ...
  test('root workflow bundle has OpenAI metadata for Codex skill browsing', () => {
    ...
    expect(content).toContain('display_name: "workflow"');
    expect(content).toContain('Use $workflow to locate the bundled workflow skills.');
  });

  test('codexSkillName mapping: root is workflow, others are workflow-{dir}', () => {
    // Root → workflow
    expect(fs.existsSync(path.join(AGENTS_DIR, 'workflow', 'SKILL.md'))).toBe(true);
    // Subdirectories → workflow-{dir}
    expect(fs.existsSync(path.join(AGENTS_DIR, 'workflow-review', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'workflow-ship', 'SKILL.md'))).toBe(true);
    // No accidental double-prefixing for workflow-* skills
    expect(fs.existsSync(path.join(AGENTS_DIR, 'workflow-workflow-review', 'SKILL.md'))).toBe(false);
  });
  ...
  test('/codex skill excluded from Codex output', () => {
    expect(fs.existsSync(path.join(AGENTS_DIR, 'workflow-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'workflow-codex'))).toBe(false);
  });

  test('Codex review step stripped from Codex-host ship and review', () => {
    const shipContent = fs.readFileSync(path.join(AGENTS_DIR, 'workflow-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(AGENTS_DIR, 'workflow-review', 'SKILL.md'), 'utf-8');
  ...
  test('multiline descriptions preserved in Codex output', () => {
    ...
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'workflow-office-hours', 'SKILL.md'), 'utf-8');
    const fmEnd = content.indexOf('\n---', 4);
    const frontmatter = content.slice(4, fmEnd);
  ...
  test('hook skills have safety prose and no hooks: in frontmatter', () => {
    const HOOK_SKILLS = ['workflow-careful', 'workflow-freeze', 'workflow-guard'];
  ...
  test('Codex preamble resolves runtime assets from repo-local or global workflow roots', () => {
    // Check a skill that has a preamble (review is a good candidate)
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'workflow-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('WORKFLOW_ROOT');
    expect(content).toContain('$_ROOT/.agents/skills/workflow');
    expect(content).toContain('$WORKFLOW_BIN/workflow-config');
    expect(content).not.toContain('~/.codex/skills/workflow/bin/workflow-config get telemetry');
  });

  // ─── Path rewriting regression tests ─────────────────────────

  test('sidecar paths point to .agents/skills/workflow/review/ (not workflow-review/)', () => {
    // Regression: gen-skill-docs rewrote .claude/skills/review → .agents/skills/workflow-review
    // but setup puts sidecars under .agents/skills/workflow/review/. Must match setup layout.
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'workflow-review', 'SKILL.md'), 'utf-8');
    // Correct: references to sidecar files use workflow/review/ path
    expect(content).toContain('.agents/skills/workflow/review/checklist.md');
    expect(content).toContain('.agents/skills/workflow/review/design-checklist.md');
    // Wrong: must NOT reference workflow-review/checklist.md (file doesn't exist there)
    expect(content).not.toContain('.agents/skills/workflow-review/checklist.md');
    expect(content).not.toContain('.agents/skills/workflow-review/design-checklist.md');
  });

  test('sidecar paths in ship skill point to workflow/review/ for pre-landing review', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'workflow-ship', 'SKILL.md'), 'utf-8');
    // Ship references the review checklist in its pre-landing review step
    if (content.includes('checklist.md')) {
      expect(content).toContain('.agents/skills/workflow/review/');
      expect(content).not.toContain('.agents/skills/workflow-review/checklist');
    }
  });
  ...
  test('greptile-triage sidecar path is correct', () => {
    ...
    if (content.includes('greptile-triage')) {
      expect(content).toContain('.agents/skills/workflow/review/greptile-triage.md');
      expect(content).not.toContain('.agents/skills/workflow-review/greptile-triage');
    }
  });
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 1: ~/.claude/skills/workflow → $WORKFLOW_ROOT
    expect(content).not.toContain('~/.claude/skills/workflow');
    expect(content).toContain('$WORKFLOW_ROOT');

    // Rule 2: .claude/skills/workflow → .agents/skills/workflow
    expect(content).not.toContain('.claude/skills/workflow');

    // Rule 3: .claude/skills/review → .agents/skills/workflow/review
  ...
  test('path rewrite rules apply to all Codex skills with sidecar references', () => {
    ...
    for (const skill of CODEX_SKILLS) {
      ...
      expect(content).not.toContain('~/.claude/skills');
      expect(content).not.toContain('.claude/skills');
      if (content.includes('workflow-config') || content.includes('workflow-telemetry-log')) {
        expect(content).toContain('$WORKFLOW_ROOT');
      }
      ...
      if (content.includes('checklist.md') && !content.includes('design-checklist.md')) {
        expect(content).not.toContain('workflow-review/checklist.md');
      }
    }
  });
  ...
  test('Claude output unchanged: review skill still uses .claude/skills/ paths', () => {
    ...
    expect(content).toContain('~/.claude/skills/workflow');
  ...
  test('codex host produces empty outside voices in design-review', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'workflow-design-review', 'SKILL.md'), 'utf-8');
    expect(codexContent).not.toContain('Design Outside Voices');
  });

  test('codex host does not include Codex design block in ship', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'workflow-ship', 'SKILL.md'), 'utf-8');
...
describe('setup script validation', () => {
  ...
  test('Codex install uses link_codex_skill_dirs', () => {
    ...
    expect(codexSection).not.toContain('ln -snf "$WORKFLOW_DIR" "$CODEX_WORKFLOW"');
  });
  ...
  test('setup separates install path from source path for symlinked repo-local installs', () => {
    expect(setupContent).toContain('INSTALL_WORKFLOW_DIR=');
    expect(setupContent).toContain('SOURCE_WORKFLOW_DIR=');
    expect(setupContent).toContain('INSTALL_SKILLS_DIR=');
    expect(setupContent).toContain('CODEX_WORKFLOW="$INSTALL_WORKFLOW_DIR"');
    expect(setupContent).toContain('link_codex_skill_dirs "$SOURCE_WORKFLOW_DIR" "$CODEX_SKILLS"');
  });
  ...
  test('Codex installs always create sidecar runtime assets for the real skill target', () => {
    ...
    expect(setupContent).toContain('create_agents_sidecar "$SOURCE_WORKFLOW_DIR"');
  });
  ...
  test('link_codex_skill_dirs reads from .agents/skills/', () => {
    ...
    expect(fnBody).toContain('workflow*');
  });

  test('link_claude_skill_dirs creates relative symlinks', () => {
    // Claude links should be relative: ln -snf "workflow/skill_name"
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('ln -snf "workflow/$skill_name"');
  });
  ...
  // T1: Sidecar skip guard — prevents .agents/skills/workflow from being linked as a skill
  test('link_codex_skill_dirs skips the workflow sidecar directory', () => {
    const fnStart = setupContent.indexOf('link_codex_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('done', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('[ "$skill_name" = "workflow" ] && continue');
  });

  // T2: Dynamic $WORKFLOW_ROOT paths in generated Codex preambles
  test('generated Codex preambles use dynamic WORKFLOW_ROOT paths', () => {
    const codexSkillDir = path.join(ROOT, '.agents', 'skills', 'workflow-ship');
    ...
    expect(content).toContain('WORKFLOW_ROOT=');
    expect(content).toContain('$WORKFLOW_BIN/');
  });
  ...
  test('setup supports --host kiro with install section and sed rewrites', () => {
    ...
    expect(setupContent).toContain('~/.kiro/skills/workflow');
  });
  ...
  test('create_codex_runtime_root exposes only runtime assets', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('workflow/SKILL.md');
    expect(fnBody).toContain('workflow_dir/VERSION');
    ...
    expect(fnBody).not.toContain('ln -snf "$workflow_dir" "$codex_runtime_root"');
  });

  test('direct Codex installs are migrated out of ~/.codex/skills/workflow', () => {
    expect(setupContent).toContain('migrate_direct_codex_install');
    expect(setupContent).toContain('$HOME/.workflow/repos/workflow');
...
describe('local analytics preamble', () => {
  test('generated SKILL.md keeps local analytics start block', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('skill-usage.jsonl');
    expect(content).toContain('mkdir -p ~/.workflow/analytics');
    expect(content).not.toContain('workflow-config get telemetry');
  });

  test('generated SKILL.md no longer contains telemetry opt-in prompt', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).not.toContain('.telemetry-prompted');
    expect(content).not.toContain('Help workflow get better');
    expect(content).not.toContain('workflow-config set telemetry community');
    expect(content).not.toContain('workflow-config set telemetry anonymous');
    expect(content).not.toContain('workflow-config set telemetry off');
  });

  test('generated SKILL.md no longer contains telemetry epilogue', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).not.toContain('Telemetry (run last)');
    expect(content).not.toContain('workflow-telemetry-log');
