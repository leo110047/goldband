...
const scriptPath = join(import.meta.dir, "..", "bin", "workflow-global-discover.ts");

describe("workflow-global-discover", () => {
  describe("normalizeRemoteUrl", () => {
    // Dynamically import to test the exported function
    let normalizeRemoteUrl: (url: string) => string;

    beforeEach(async () => {
      const mod = await import("../bin/workflow-global-discover.ts");
    ...
    test("SSH and HTTPS for same repo normalize to same URL", () => {
      const ssh = normalizeRemoteUrl("git@github.com:garrytan/workflow.git");
      const https = normalizeRemoteUrl("https://github.com/garrytan/workflow.git");
      const httpsNoDotGit = normalizeRemoteUrl("https://github.com/garrytan/workflow");
