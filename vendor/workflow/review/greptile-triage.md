...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/workflow/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.workflow/projects/$REMOTE_SLUG/greptile-history.md"
```
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/workflow/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.workflow/projects/$REMOTE_SLUG"
mkdir -p ~/.workflow
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `~/.workflow/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.workflow/greptile-history.md` (global aggregate)
