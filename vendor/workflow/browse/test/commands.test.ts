...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/workflow-cli-');
    const execPath = path.join(root, '.claude/skills/workflow/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/workflow/browse/src/server.ts');
