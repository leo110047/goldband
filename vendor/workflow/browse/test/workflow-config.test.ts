/**
 * Tests for bin/workflow-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * WORKFLOW_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'workflow-config');
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'workflow-config-test-'));
});
...
describe('workflow-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { WORKFLOW_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
