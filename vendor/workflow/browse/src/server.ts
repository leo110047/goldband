/**
 * workflow browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.workflow/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.workflow/browse-{console,network,dialog}.log
...
function generateHelpText(): string {
  ...
  const lines = ['workflow browse — headless browser for AI agents', '', 'Commands:'];
...
const SESSIONS_DIR = path.join(process.env.HOME || '/tmp', '.workflow', 'sidebar-sessions');
...
function findBrowseBin(): string {
  const candidates = [
    ...
    path.resolve(__dirname, '..', '..', '.claude', 'skills', 'workflow', 'browse', 'dist', 'browse'),
    path.join(process.env.HOME || '', '.claude', 'skills', 'workflow', 'browse', 'dist', 'browse'),
  ];
...
function createWorktree(sessionId: string): string | null {
  try {
    ...
    const worktreeDir = path.join(process.env.HOME || '/tmp', '.workflow', 'worktrees', sessionId.slice(0, 8));
...
function spawnClaude(userMessage: string, extensionUrl?: string | null, forTabId?: number | null): void {
  ...
  const agentQueue = process.env.SIDEBAR_QUEUE_PATH || path.join(process.env.HOME || '/tmp', '.workflow', 'sidebar-agent-queue.jsonl');
  const workflowDir = path.dirname(agentQueue);
...
function killAgent(targetTabId?: number | null): void {
  ...
  const cancelDir = path.join(process.env.HOME || '/tmp', '.workflow');
...
// so this branch is the normal path for `$B connect`.
...
if (BROWSE_PARENT_PID > 0 && !IS_HEADED_WATCHDOG) {
  ...
  setInterval(() => {
    try {
      ...
      //    /pair-agent or `$B connect` sessions.
...
async function shutdown(exitCode: number = 0) {
  ...
  const profileDir = path.join(process.env.HOME || '/tmp', '.workflow', 'chromium-profile');
...
async function start() {
  ...
  const server = Bun.serve({
    ...
    fetch: async (req) => {
      ...
      // Welcome page — served when Workflow Browser launches in headed mode
      if (url.pathname === '/welcome') {
        const welcomePath = (() => {
          // Check project-local designs first, then global
          const slug = process.env.WORKFLOW_SLUG || 'unknown';
          const homeDir = process.env.HOME || process.env.USERPROFILE || '/tmp';
          const projectWelcome = `${homeDir}/.workflow/projects/${slug}/designs/welcome-page-20260331/finalized.html`;
          if (fs.existsSync(projectWelcome)) return projectWelcome;
          // Fallback: built-in welcome page from workflow install
          const skillRoot = process.env.WORKFLOW_SKILL_ROOT || `${homeDir}/.claude/skills/workflow`;
        ...
        return new Response(
          `<!DOCTYPE html><html><head><title>Workflow Browser</title>
          ...
          <body><div class="msg"><div class="gold">◈</div><p>Workflow Browser ready.</p><p style="font-size:.85em">Waiting for commands from Claude Code.</p></div></body></html>`,
      ...
      if (url.pathname === '/tunnel/start' && req.method === 'POST') {
        ...
        try {
          // Read ngrok authtoken: env var > ~/.workflow/ngrok.env > ngrok native config
          let authtoken = process.env.NGROK_AUTHTOKEN;
          if (!authtoken) {
            const ngrokEnvPath = path.join(process.env.HOME || '', '.workflow', 'ngrok.env');
  ...
  // Reads NGROK_AUTHTOKEN from env or ~/.workflow/ngrok.env.
  ...
  if (process.env.BROWSE_TUNNEL === '1') {
    try {
      ...
      if (!authtoken) {
        console.error('[browse] BROWSE_TUNNEL=1 but no NGROK_AUTHTOKEN found. Set it via env var or ~/.workflow/ngrok.env');
      } else {
        const ngrok = await import('@ngrok/ngrok');
        const domain = process.env.NGROK_DOMAIN;
