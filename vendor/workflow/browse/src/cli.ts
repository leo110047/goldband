/**
 * workflow CLI — thin wrapper that talks to the persistent server
 *
 * Flow:
 *   1. Read .workflow/browse.json for port + token
...
function cleanupLegacyState(): void {
  // No legacy state on Windows — /tmp and `ps` don't exist, and workflow
...
async function ensureServer(): Promise<ServerState> {
  ...
  if (process.env.BROWSE_NO_AUTOSTART === '1') {
    ...
    console.error('[browse] The headed browser may have been closed. Run `$B connect` to restart.');
    process.exit(1);
  }
...
/** Check if ngrok is installed and authenticated (native config or workflow env). */
function isNgrokAvailable(): boolean {
  // Check workflow's own ngrok env
  const ngrokEnvPath = path.join(process.env.HOME || '/tmp', '.workflow', 'ngrok.env');
...
async function handlePairAgent(state: ServerState, args: string[]): Promise<void> {
  ...
  if (localHost) {
    try {
      ...
      let globalRoot = `.${localHost}/skills/workflow`;
      try {
...
async function main() {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log(`workflow browse — Fast headless browser for AI coding agents
  ...
  if (command === 'connect') {
    ...
    try {
      ...
      try {
        ...
        const agentQueue = path.join(process.env.HOME || '/tmp', '.workflow', 'sidebar-agent-queue.jsonl');
        try {
 ...
 command: 'disconnect', args: [] }),
  ...
  if (command === 'pair-agent') {
    ...
    if (state.mode !== 'headed' && !hasFlag(commandArgs, '--headless')) {
      console.log('[browse] Opening Workflow Browser so you can see what the remote agent does...');
